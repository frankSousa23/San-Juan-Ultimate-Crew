...
export const RESOURCE_DOCS: Record<number | string, ResourceDocumentConfig> = {
  1: {
    id: 1,
    title: 'Reglamento Oficial de Ultimate Frisbee (WFDF 2025 / 2026)',
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          ...
          '• Disco Reglamentario: Disco volador de 175 gramos con 27 cm de diámetro (estándar Discraft Ultra-Star o modelo homologado por WFDF).',
      ...
      },
      {
        ...
        body: [
          ...
          '• Rotación de Posesión (Turnover): El disco pasa inmediatamente al equipo contrario si toca el suelo (pase incompleto), es interceptado, sale de los límites del campo (Out of bounds), es bloqueado o cae por falta ofensiva.',
        ],
      },
      {
        heading: '4. Anotación, Saque Inicial (Pull) y Duración',
        body: [
          ...
          '• Puntuación de Partido: Partidos oficiales se juegan a 15 puntos con límite de tiempo de 90 a 100 minutos y medio tiempo al llegar al punto 8.',
        ],
      },
    ],
    keyTakeaway: 'Principio Clave: La honestidad y el autocontrol son tan valiosos como la habilidad atlética. Conocer el reglamento a fondo evita discusiones prolongadas y eleva el nivel competitivo.',
  },
  2: {
    id: 2,
    ...
    sections: [
      {
        ...
        body: [
          ...
          'Al finalizar cada encuentro, ambos equipos se reúnen en el "Círculo de Espíritu" para dialogar constructivamente y posteriormente completar la rúbrica oficial de 5 categorías (puntuación de 0 a 4 por categoría, total base 10/20).',
        ],
      },
      {
    ...
    keyTakeaway: 'Rúbrica SOTG: 10 puntos representa un partido limpio y acorde a las normas internacionales. Puntuaciones mayores a 10 premian conductas sobresalientes de caballerosidad.',
  },
  3: {
    id: 3,
    ...
    keyTakeaway: 'Recomendación para Mesa Técnica: Los anotadores deben confirmar con señas visibles hacia los capitanes cada gol validado y tiempo muerto concedido.',
  },
  4: {
    id: 4,
    ...
    keyTakeaway: 'Regla de Oro del Lanzador: La rotación ("Spin") estabiliza el vuelo contra ráfagas de viento. Prioriza la velocidad de muñeca sobre la fuerza bruta de brazo.',
  },
  5: {
    id: 5,
    ...
    keyTakeaway: 'Nutrición en Torneo: Los atletas que reponen electrolitos de forma constante mantienen una capacidad de sprint 30% superior en los partidos de la tarde.',
  },
}

// Normalizador de claves
function resolveConfig(docInput: number | string | any): ResourceDocumentConfig {
  if (typeof docInput === 'object' && docInput !== null && docInput.title) {
    // Si ya tiene estructura de recurso
    return {
      id: docInput.id || 1,
      title: docInput.title,
      category: docInput.category || 'Recurso Técnico',
      subtitle: docInput.subtitle || docInput.description || 'Documento Oficial del Sistema SIGEDIVO',
      sections: docInput.sections || [
        {
          heading: 'Descripción General',
          body: [docInput.description || 'Sin descripción detallada.'],
        },
      ],
      keyTakeaway: docInput.keyTakeaway || 'Documento oficial registrado en la plataforma SIGEDIVO.',
    }
  }

  const rawId = String(docInput)
  const numId = parseInt(rawId, 10)

  if (RESOURCE_DOCS[rawId]) return RESOURCE_DOCS[rawId]
  if (RESOURCE_DOCS[numId]) return RESOURCE_DOCS[numId]

  if (numId >= 501 && numId <= 505) {
    const mapped = numId - 500
    if (RESOURCE_DOCS[mapped]) return RESOURCE_DOCS[mapped]
  }

  // Fallback seguro
  return RESOURCE_DOCS[1]
}

export function generateResourcePdf(docInput: number | string | any): jsPDF {
  const config = resolveConfig(docInput)
  const doc = new jsPDF({
  ...
  // Banda de título
  const titleLines = doc.splitTextToSize(config.title, contentWidth - 10)
  const subLines = doc.splitTextToSize(config.subtitle, contentWidth - 10)
  const headerCardHeight = 12 + titleLines.length * 5 + 3 + subLines.length * 3.8 + 4

  doc.setFillColor(241, 245, 249)
  doc.roundedRect(margin, 17, contentWidth, headerCardHeight, 2, 2, 'F')
  doc.setDrawColor(203, 213, 225)
  doc.setLineWidth(0.3)
  doc.rect(margin, 17, contentWidth, headerCardHeight, 'S')

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(12.5)
  doc.setTextColor(15, 23, 42)
  doc.text(titleLines, margin + 5, 24)

  doc.setFont('helvetica', 'italic')
  doc.setFontSize(8.5)
  doc.setTextColor(71, 85, 105)
  doc.text(subLines, margin + 5, 24 + titleLines.length * 5 + 1)

  let curY = 17 + headerCardHeight + 5

  config.sections.forEach((sec) => {
    // Calculamos si la sección cabe en la página
    ...
    doc.roundedRect(margin, curY, contentWidth, 6.5, 1, 1, 'F')
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(9.5)
    doc.setTextColor(30, 58, 138)
    doc.text(sec.heading, margin + 3.5, curY + 4.6)
    curY += 9
    ...
    sec.body.forEach((para) => {
      ...
      if (curY + lines.length * 3.8 > pageHeight - 20) {
        addFooter()
        doc.addPage()
        addHeader()
        curY = 18
      ...
      curY += lines.length * 3.8 + 2.5
    })

    curY += 2
  })

  // Cuadro de Takeaway / Conclusión
  if (config.keyTakeaway) {
    const takeawayLines = doc.splitTextToSize(config.keyTakeaway, contentWidth - 10)
    const takeawayH = 8 + takeawayLines.length * 3.8

    if (curY + takeawayH > pageHeight - 20) {
      addFooter()
      doc.addPage()
      addHeader()
      curY = 18
    ...
    doc.setFillColor(240, 253, 244) // Emerald claro
    doc.setDrawColor(187, 247, 208)
    doc.setLineWidth(0.3)
    doc.roundedRect(margin, curY, contentWidth, takeawayH, 1.5, 1.5, 'FD')

    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8)
    doc.setTextColor(22, 101, 52)
    doc.text(takeawayLines, margin + 5, curY + 5.5)
...
export function downloadResourcePdf(docInput: number | string | any, filename?: string) {
  const doc = generateResourcePdf(docInput)
  const cfg = resolveConfig(docInput)
  const cleanTitle = (cfg.title || 'Documento_SIGEDIVO')
    .replace(/[^a-zA-Z0-9_-]/g, '_')
    .substring(0, 40)
  const fname = filename || `${cleanTitle}.pdf`
  doc.save(fname)
}
