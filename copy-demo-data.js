...
function replaceMockArray(name, guestArrayName, idKey) {
  ...
  if (content) {
    ...
    const regex = new RegExp(`this\\.${name} = \\[[\\s\\S]*?\\];`, 'm');
    if (regex.test(mockDb)) {
      mockDb = mockDb.replace(regex, `this.${name} = [${cleanContent}];`);
    } else {
      console.log(`Could not find this.${name} in mockDb.ts`);
    }
    ...
    console.log(`Could not extract ${guestArrayName}`);
  }
}
...
replaceMockArray('posts', 'GUEST_POSTS', 'post');

fs.writeFileSync('apps/api/src/lib/mockDb.ts', mockDb);
console.log('Restored all demo data to mockDb.ts successfully.');
