...
export const RESOURCE_DOCS: Record<number | string, ResourceDocumentConfig> = {
  1: {
    ...
    subtitle: 'Normativa Oficial Internacional de la World Flying Disc Federation para Ultimate Frisbee en Césped y Playa',
    sections: [
      {
        ...
        body: [
          ...
          '• Deporte sin Contacto Físico: Está prohibido el contacto corporal intencional. Toda colisión que afecte el juego de otro participante constituye una falta (foul).',
          '• Autogestión y Resolución de Disputas: Solo los atletas directamente involucrados en una jugada discuten las llamadas. Si no hay acuerdo tras 15 segundos de diálogo respetuoso, el disco regresa al lanzador anterior (Contested Call).',
        ],
      },
      {
        heading: '2. Dimensiones Oficiales del Campo y Equipamiento',
        body: [
          '• Campo de Césped (Outdoor): Longitud total de 100 metros (64 metros de campo central más dos zonas de anotación de 18 metros en cada extremo). Ancho reglamentario de 37 metros.',
          '• Puntos de Marca de Brick: Situados a 18 metros de cada línea de gol en el centro del campo.',
          '• Disco Reglamentario: Disco volador homologado de 175 gramos con 27 cm de diámetro (estándar Discraft Ultra-Star o homologado por WFDF).',
      ...
      },
      {
        ...
        body: [
          ...
          '• Conteo de Stall: El defensa que marca al poseedor (Mark) debe situarse a al menos un disco de distancia y contar audiblemente en voz alta: "Stalling 1, 2, 3... hasta 10". Si llega a 10 antes del lanzamiento, ocurre rotación inmediata (Stall Out).',
      ...
      },
      {
        ...
        body: [
          '• Cómo Anotar: Se consigue un punto cuando un atacante atrapa un pase reglamentario dentro de la zona de gol (Endzone) rival con al menos un punto de contacto inicial válido dentro del área.',
...
export function generateResourcePdf(docId: number | string): jsPDF {
  ...
  function addHeader() {
    ...
    doc.text('SIGEDIVO • DOCUMENTO TÉCNICO OFICIAL DE ULTIMATE FRISBEE', margin, 7.5)
  ...
  function addFooter() {
    ...
    doc.text('Biblioteca Técnica Oficial WFDF / SIGEDIVO 2026', margin, pageHeight - 7)
  ...
  const titleBandHeight = Math.max(28, 12 + titleLines.length * 5.5 + subLines.length * 4.2)

  doc.setFillColor(241, 245, 249)
  doc.roundedRect(margin, 16, contentWidth, titleBandHeight, 2, 2, 'F')
  doc.setDrawColor(203, 213, 225)
  doc.setLineWidth(0.3)
  doc.roundedRect(margin, 16, contentWidth, titleBandHeight, 2, 2, 'S')

  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  doc.setTextColor(15, 23, 42)
  doc.text(titleLines, margin + 4, 23)

  doc.setFont('helvetica', 'italic')
  doc.setFontSize(8.5)
  doc.setTextColor(71, 85, 105)
  doc.text(subLines, margin + 4, 25 + titleLines.length * 5.5)

  let curY = 16 + titleBandHeight + 6

  config.sections.forEach((sec) => {
    // Verificar si necesitamos nueva página para el encabezado
    ...
    doc.text(sec.heading, margin + 3, curY + 4.8)
    curY += 10
    ...
    sec.body.forEach((para) => {
      ...
      if (curY + lines.length * 4.4 > pageHeight - 18) {
        addFooter()
        doc.addPage()
        addHeader()
        curY = 20
...
export function downloadResourcePdf(docId: number | string, filename?: string) {
  ...
  const fname =
    filename ||
    (cfg
      ? `${cfg.title.replace(/[^a-zA-Z0-9_-]/g, '_').substring(0, 40)}.pdf`
      : `Documento_SIGEDIVO_${docId}.pdf`)
  doc.save(fname)
}
