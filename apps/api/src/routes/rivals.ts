...
router.get('/:id/stats', asyncHandler(async (req: Request, res: Response) => {
  ...
  const playerStats = annotations
    ...
    .reduce((acc, ann) => {
      ...
      acc[playerId].total++
      if (ann.type === 'GOAL') acc[playerId].goals++
      if (ann.type === 'ASSIST') acc[playerId].assists++
      if (ann.type === 'DEFENSE') acc[playerId].interceptions++
      return acc
    }, {} as Record<number, { player: any; goals: number; assists: number; interceptions: number; total: number }>)
