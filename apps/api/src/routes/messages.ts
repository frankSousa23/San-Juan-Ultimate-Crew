...
router.get('/', async (req: Request, res: Response) => {
  ...
  if (!Number.isInteger(channelId)) return res.status(400).json({ error: 'Invalid channelId' })
  if (Number.isNaN(limit) || limit < 1 || limit > 100) return res.status(400).json({ error: 'Invalid limit' })
  try {
